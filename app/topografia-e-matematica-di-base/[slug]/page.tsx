...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'topografia-e-matematica-di-base', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/" },
      { name: "Topografia e Matematica di Base", path: "/topografia-e-matematica-di-base" },
      { name: calculatorName }
  ];
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {/* Calculator */}
            <div className="lg:col-span-2 p-1 bg-white rounded-2xl shadow-lg">
                <CalculatorComponent />
            </div>
