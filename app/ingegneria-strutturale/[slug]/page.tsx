...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'elettrotecnica-ed-elettricita', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/" },
      { name: "Elettrotecnica ed Elettricità", path: "/elettrotecnica-ed-elettricita" },
      { name: calculatorName }
  ];
