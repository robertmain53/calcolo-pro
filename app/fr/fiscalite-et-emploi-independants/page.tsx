...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'fr', 'fiscalite-et-emploi-independants');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Fiscalité et emploi indépendants";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/fr/fiscalite-et-emploi-independants/${calc.slug}`} 
