...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'fr', 'fiscalite-et-emploi-independants', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/fr" },
      { name: "Fiscalité et emploi indépendants", path: "/fr/fiscalite-et-emploi-independants" },
      { name: calculatorName }
  ];
