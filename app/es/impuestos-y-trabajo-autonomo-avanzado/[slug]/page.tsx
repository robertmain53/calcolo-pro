...
async function getContent(slug: string) {
  try {
    const contentPath = path.join(process.cwd(), 'content', 'es', 'impuestos-y-trabajo-autonomo-avanzado', `${slug}.md`);
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const crumbs = [
      { name: "Home", path: "/es" },
      { name: "Impuestos y trabajo autónomo (avanzado)", path: "/es/impuestos-y-trabajo-autonomo-avanzado" },
      { name: calculatorName }
  ];
