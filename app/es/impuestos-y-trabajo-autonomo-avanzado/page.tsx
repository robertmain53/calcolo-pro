...
async function getCalculators() {
    const calculatorsPath = path.join(process.cwd(), 'content', 'es', 'impuestos-y-trabajo-autonomo-avanzado');
    try {
...
export default async function CategoryPage() {
  ...
  const categoryName = "Impuestos y trabajo autónomo (avanzado)";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {calculators.map((calc) => (
            <Link 
              key={calc.slug} 
              href={`/es/impuestos-y-trabajo-autonomo-avanzado/${calc.slug}`} 
